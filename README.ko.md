...
현재 `0.8.0` 코어는 Vite + Phaser, Godot 4, LOVE2D, PixiJS + Vite, Plain Canvas + Vite, Unity 프로젝트에서 sprite 이미지 핸드오프와 audio, atlas/tileset, UI 에셋 파이프라인을 지원합니다.
...
`--target phaser`, `--target godot`, `--target love2d`, `--target pixi`, `--target canvas`, `--target unity`를 사용할 수 있습니다.
...
- Phaser, Godot, LOVE2D, PixiJS, Canvas, Unity 어댑터.
