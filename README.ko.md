...
현재 `0.9.2` 코어는 Vite + Phaser, Godot 4, LOVE2D, PixiJS + Vite, Three.js + Vite, Plain Canvas + Vite, Unity 프로젝트에서 sprite 이미지 핸드오프, visual reference 기록, motion 분석, animation compile/install, audio, atlas/tileset, UI 에셋 파이프라인과 agent loop task packet을 지원합니다.
...
- 스케치, 목업, 컨셉 이미지, 로컬 파일, URL을 안전하게 남기는 visual reference 기록. URL은 다운로드하지 않고 provenance만 저장한다.
...
- audio, atlas/tileset, UI compile/install/verify/report/rollback을 같은 로컬 run-state 파이프라인으로 처리.
...
- 사용자가 스케치, 목업, 컨셉 이미지, 영상 URL, 로컬 reference file을 주면 `ingest reference --json`으로 기록합니다.
