...
현재 `0.7.3` 코어는 Vite + Phaser, Godot 4, LOVE2D, PixiJS + Vite, Plain Canvas + Vite에서 sprite 이미지 핸드오프와 P4 audio, atlas/tileset, UI 에셋 파이프라인을 지원합니다.
...
- P4 audio, atlas/tileset, UI compile/install/verify/report/rollback을 같은 로컬 run-state 파이프라인으로 처리.
