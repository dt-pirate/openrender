...
현재 `1.1.0` 코어는 Vite + Phaser, Godot 4, LOVE2D, PixiJS + Vite, Three.js + Vite, Plain Canvas + Vite, Unity 프로젝트에서 스프라이트 이미지 핸드오프, 시각 레퍼런스 기록, 시각 메모리 누적, 제작자 취향 연속성, 초점별 메모리 브리프, run 드리프트 리뷰, 모션 분석, 애니메이션 컴파일/설치, 오디오, 아틀라스/타일셋, UI 에셋 파이프라인, 루프 실행 이력 기록, 엔진별 작업 패킷, 루프 완료 기록, 로컬 서비스 스냅샷, 선택형 런타임/빌드 스모크 체크를 지원합니다.
