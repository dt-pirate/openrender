...
- Engine readiness verification checks for Phaser, Godot, LOVE2D, PixiJS, Three.js, Canvas, and Unity, explicit runtime smoke semantics for Godot/LOVE2D, and opt-in web build smoke for Vite targets.
- Local-only `service snapshot --json` export for future dashboards and agent supervisors without adding hosted APIs.
