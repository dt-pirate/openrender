...
当前 `1.1.0` 核心支持 Vite + Phaser、Godot 4、LOVE2D、PixiJS + Vite、Three.js + Vite、Plain Canvas + Vite、Unity 项目的精灵图片交接、视觉参考记录、视觉记忆累积、创作者偏好延续、按焦点生成的记忆 brief、run drift review、动作分析、动画编译/安装，以及音频、图集/瓦片集、UI 资产管线、循环执行记录、引擎任务包、循环完成记录、本地服务快照和可选 runtime/build smoke checks。
...
- 从 run、loop、用户反馈和视觉参考中派生 project card、agent card、user-direction card、engine card、creator-taste card、game-direction card、visual-avoidance card 和 visual-evidence index 的记忆基础设施。
- 通过 `memory query` 和 `memory review`，在下一次代理任务前简短检查创作者偏好、游戏方向、视觉规避和 drift signal。
...
- 下一步工作依赖 visual style 或操作手感时，先运行 `memory query --for style|ui|movement --json --compact` 检查创作者偏好和视觉证据。
- 安装或 loop 之后运行 `memory review --run latest --json`，检查记忆不足或方向偏离信号。
