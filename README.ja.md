...
現在の `0.8.0` コアは、Vite + Phaser、Godot 4、LOVE2D、PixiJS + Vite、Plain Canvas + Vite、Unity プロジェクトで sprite 画像ハンドオフと audio、atlas/tileset、UI アセットパイプラインをサポートしています。
...
`--target phaser`、`--target godot`、`--target love2d`、`--target pixi`、`--target canvas`、`--target unity` を使用できます。
...
- Phaser、Godot、LOVE2D、PixiJS、Canvas、Unity 向けアダプター。
